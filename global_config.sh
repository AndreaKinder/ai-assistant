...
install_dependencies() {
  ...
  # Paquetes necesarios
  local dependencies=("curl" "jq" "python3" "pip" "npm" "nodejs" "neovim")
...
# Funcion para editar archivo de MCP
edit_mcp_config() {
  local mcp_config_file="$CONFIG_DIR/mcp-config.json"
  if [ -f "$mcp_config_file" ]; then
    nvim "$mcp_config_file"
  else
    info_message "El archivo de configuración de MCP no existe."
  fi
}
...
EOF
  ...
  while true; do
    ai_message "¿Desea editar la configuración de MCP? (s/n)"
    read -r install_mcp

    if [[ "$install_mcp" == "s" ]]; then
      edit_mcp_config
      echo -e "${CYAN}Configuración de MCP editada.${RESET}"
      break
    elif [[ "$install_mcp" == "n" ]]; then
      info_message "No se editará la configuración de MCP."
      break
    else
      error_message "Entrada inválida. Por favor, ingrese 's' para sí o 'n' para no."
    fi
  done
...
process_with_gemini() {
  ...
  # Uso de jq para manejar correctamente el escape de la respuesta
  updated_history=$(echo "$updated_history" | jq --arg resp "$assistant_response" '. + [{"role":"model","parts":[{"text":$resp}]}]')
  echo "$updated_history" >"$memory_file"
